...
export class CheckoutComponent implements OnInit {
  ...
  placeOrder(){
    ...
    const {firstname,lastname,country,address,town,state,postCode,phone,email,note,bank} = this.orderForm;  
...
console.log("Username:", this.username);
    this.orderService.placeOrder(firstname,lastname,country,address,town,state,this.totalFinal,postCode,phone,email,note,bank,this.voucher.money,this.listOrderDetail,this.username).subscribe({
      next: res => {
        console.log(res);
      if (bank === 0) {
